...
- The shared preprocessor splits continuous vs. binary features, imputes them (median/most-frequent), and standardizes every continuous column with `StandardScaler`. That column-wise scaling already keeps age-like variables in a comparable range, so avoid adding per-row norms or extra min-max steps unless a new model explicitly needs them.  
...
- Processing intensity is selected right on the Upload step (`ProcessingModeSelector` in `components/common`). Light preserves the legacy grids, Hard expands each backend grid (and estimator alongside the ETA costs), and Custom renders per-model inputs in `TrainingConfigStep` and forwards the values through `types.ts` → `services/mlService.ts` → `backend/services/training.py`. Keep these layers synchronized whenever you add/remove hyperparameters.

## Build, Test, and Development Commands
Run `npm install` once, then `npm run dev` for hot reload on `http://localhost:3000`. Verify release output with `npm run build && npm run preview`, and execute `npm test` (Vitest + RTL) for the UI checks. Backend work starts with `python -m venv .venv && source .venv/bin/activate && pip install -r backend/requirements.txt`, and `uvicorn backend.main:app --host 0.0.0.0 --port 8000 --reload` serves the API plus `/docs`. Use `python launcher.py` whenever you need to mimic the turnkey workflow.
